...
public class ShapeBuilder implements IShape {
    ...
    public ShapeBuilder Radius(float topleft, float topright, float botleft, float botright) {
        drawable.setCornerRadii(new float[]{topleft, topleft, topright, topright, botright, botright, botleft,
                botleft});
        if (container != null) {
    ...
    public ShapeBuilder Gradient(int... color) {
        return GradientInit(GradientDrawable.Orientation.TOP_BOTTOM, color);
    }
    ...
    private ShapeBuilder GradientInit(GradientDrawable.Orientation orientation, int... color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            drawable.setOrientation(orientation);
            drawable.setColors(color);
        } else {
            isOperate = true;
            drawable = new GradientDrawable(orientation, color);
        }
        return this;
    }
