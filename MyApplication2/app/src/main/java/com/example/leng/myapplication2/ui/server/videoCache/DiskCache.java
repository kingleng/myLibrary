...
public class DiskCache {
    ...
    public DiskCache(String cachePath, int maxSize, int cacheSlice) {
    ...
    private void checkToCombine(SegmentInfo segmentKey) {
        ...
        ArrayList<DiskCache.Range> fileRanges = new ArrayList<>();

        for (String fileName : list) {
            DiskCache.Range range = getFileRangeByName(fileName);
        ...
        for (int i = 0; i < fileRanges.size(); i++) {
            DiskCache.Range rangeI = fileRanges.get(i);
            if (rangeI.length() < cacheSlice) {
                for (int j = 0; j < fileRanges.size(); j++) {
                    DiskCache.Range rangeJ = fileRanges.get(j);
                    if (i != j) {
                        if (rangeI.isIntersected(rangeJ)) {
                            if (rangeJ.contains(rangeI)) {
                                ...
                                DiskCache.Range range = new DiskCache.Range(startRange, endRange);
                                if (file != null) {
    ...
    @SuppressWarnings("ResultOfMethodCallIgnored")
    private void deleteFileByRange(DiskCache.Range range) {
        new File(range.start + NAME_SEPARATOR + range.end).delete();
    }

    private DiskCache.Range getFileRangeByName(String name) {
        DiskCache.Range range = new DiskCache.Range();
    ...
    private File combineFile(File parentPathFile, DiskCache.Range range1, DiskCache.Range range2) {
        ...
        DiskCache.Range firstRange;
        DiskCache.Range secondRange;
    ...
    public List<CacheResult> get(SegmentInfo segmentInfo) {
        ...
        try {
            ...
            for (String localFile : localFiles) {
                DiskCache.Range range = getFileRangeByName(localFile);
    ...
    private static class Range {
        ...
        private boolean contains(DiskCache.Range range) {
            return start <= range.start && end >= range.end;
        }

        private boolean isIntersected(DiskCache.Range range) {
            int minStart = Math.min(start, range.start);
            int maxEnd = Math.max(end, range.end);
            return maxEnd - minStart + 1 < length() + range.length();
        }

        private boolean isContinuous(DiskCache.Range range) {
