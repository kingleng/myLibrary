package com.kingleng.baselibrary.face;
...
import com.kingleng.baselibrary.R;
...
public class DialogUtil {
	...
	public AlertDialog showEditText(final TextView text, final OnEditModifComplateListener listener){
		...
		builder.setPositiveButton(activity.getResources().getString(R.string.complete), new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if (str.length() <= 0){
					ConUtil.showToast(activity, activity.getResources().getString(R.string.modif_name_fail));
					return;
				}

				text.setText(str);
				if (listener != null){
					listener.onModifComplate(str);
				}
	...
	public static interface OnEditModifComplateListener{
		void onModifComplate(String name);
	}
}