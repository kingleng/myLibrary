...
package com.info.aegis.plugincorelibrary.utils;
...
public class PluginUtil {
    ...
    public static ComponentName getComponent(Intent intent) {
        ...
        if (isIntentFromPlugin(intent)) {
            return new ComponentName(intent.getStringExtra(Constants.TARGET_INTENT_PACKAGE),
                intent.getStringExtra(Constants.TARGET_INTENT_NAME));
        }
    ...
    public static int getTheme(Context context, ComponentName component) {
        PlugInfo loadedPlugin = PluginManager.getInstance().getLoadedPlugin(component);
