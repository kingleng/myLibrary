...
//    }
    ...
    public static Resources createResourcesSimple(Context hostContext, String apk) throws Exception {
        ...
        Map<String, PlugInfo> pluginListMap = PluginManager.getInstance().getPluginPkgToInfoMap();
        List<PlugInfo> pluginList = new ArrayList<PlugInfo>(pluginListMap.values());
        for (PlugInfo plugin : pluginList) {
        ...
        for (PlugInfo plugin : pluginList) {
            plugin.setResources(newResources);
        }
        hookResources(hostContext,newResources);
        return newResources;
    }
...
//            Object activityThread = FieldUtil.getField(activityManagerClazz,null,"sCurrentActivityThread");
